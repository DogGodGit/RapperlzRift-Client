...
public abstract class AuthServerCommand
{
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Constants
	...
	protected float		m_fTimeoutInterval = 0f;

	protected int		m_nStatus = kStatus_None;

	protected int					m_nResult = kResult_OK;
	protected AuthServerResponse	m_response = null;
	...
	public AuthServerCommand(string sCommand)
	{
		m_sCommand = sCommand;
	}
	...
	public virtual AuthServerResponse response
	{
	...
	protected virtual void Finish(int nResult, AuthServerResponse response, Exception error)
	{
	...
	protected abstract AuthServerResponse CreateResponse();
	...
	public virtual void Run()
	{
		if (m_nStatus != kStatus_None)
			throw new InvalidOperationException("이미 실행되었습니다.");//已经执行

        string sCommandUrl = CsConfiguration.Instance.AuthServerApiUrl;
		...
		if (joReq != null)
		{
			...
			//WebService.instance.Send (sCommandUrl, Encoding.UTF8.GetBytes (Util.GetHashValue(joReq.ToJson(), "E")), m_fTimeoutInterval, OnResult, OnError);
		}
		else
		{
	...
	protected virtual void OnResult(string sContent)
	{
		...
		AuthServerResponse response = null;
		...
		case "CreateEntermateUser":
		case "Login":
			//sContent = Util.GetHashValue(sContent, "D");
			break;
		default:
			break;
		}
