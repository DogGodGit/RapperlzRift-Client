...
namespace VLB
{
    ...
    public class VolumetricLightBeamEditor : EditorCommon
    {
        ...
        SerializedProperty colorFromLight, color;
        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUI.DisabledGroupScope(!Noise3D.isSupported))
            {
                ...
                if (noiseEnabled.boolValue)
                {
                    ...
                    EditorGUILayout.HelpBox("For some reason, Texture3D (and so 3DNoise) are not working in Editor with WebGL. But it works totally fine in the browser if you export a WebGL build :)", MessageType.Warning);
#endif
                }
            }
