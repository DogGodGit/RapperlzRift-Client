...
struct VertexOutputForwardBase
{
    ...
    #if UNITY_REQUIRE_FRAG_WORLDPOS && !UNITY_PACK_WORLDPOS_WITH_TANGENT  || PLANE_CLIPPING_ENABLED
...
half4 fragForwardBaseInternal (VertexOutputForwardBase i)
{
	...
	fixed4 m = tex2D(_MaskTex, i.tex.xy); 	
	half4 res = m;
	 	 	
	if (m.r >= .1) res = _Color1;
	if (m.g >= .1) res = _Color2;
	if (m.b >= .1) res = _Color3;
	if (m.a <= .1) res = _Color4;

	if (m.r >= .01 && m.g >= .01) res = _Color1 * m.r + _Color2 * m.g;
    if (m.b >= .01 && m.g >= .01) res = _Color2 * m.b + _Color3 * m.g;
    if (m.r >= .01 && m.b >= .01) res = _Color3 * m.b + _Color1 * m.r;


	if (m.r <= .1 && m.g <= .1 && m.b <= .1 && m.a > .1) res = 1;
    ...
    if(_RimPower != 0)
    {
    	half rim = (1.0 - saturate(dot( normalize(-s.eyeVec), s.normalWorld)));
    	c.rgb += _RimPower * (_RimColor.rgb * (_RimColor.a)) * pow(rim/_RimWidth, 10);
 	}
...
)
{
    ...
    #endif
    	half rim = (1.0 - saturate(dot( normalize(-s.eyeVec), s.normalWorld)));
		emissiveColor.rgb += _RimPower * (_RimColor.rgb * (_RimColor.a)) * pow(rim, 10);
