...
namespace Artngame.INfiniDy {
	...
	public class InfiniGRASSManagerEditor : Editor {
		...
		void OnSceneGUI () {
			...
			if (!script.UnGrown) {
				...
				if (script.CleanUp && script != null && script.Grasses.Count > 0 & !Application.isPlaying & 1==1) {
					...
					if (forest.Registered_Brances.Count < script.MinBranches && forest.Grow_tree_ended)
					{
		...
		public override void  OnInspectorGUI() {
			...
			//v1.4.6
			if (script.GrassPrefabsNames.Count == 0) {
			...
			//v1.4.6
			if (script.RocksPrefabsIcons.Count == 0 | script.RocksPrefabsIcons.Count < 3 ) {
				script.RocksPrefabsIcons.Add(RockICON1);
				script.RocksPrefabsIcons.Add(RockICON2);
				script.RocksPrefabsIcons.Add(RockICON3);
			}
			//v1.4.6
			if (script.FencePrefabsIcons.Count == 0 | script.FencePrefabsIcons.Count < 2 ) {
	...
	//		GUILayout.Box ("", GUILayout.Height (2), GUILayout.Width (410));
			...
			if (!script.UnGrown) {
				...
				EditorGUILayout.IntSlider (MassPlantDensity, 2, 20*(int)(20/capped_scale), new GUIContent ("Density of Mass Grow"));
...
//							forest.End_scale = forest.End_scale * script.Scale_grabbed;
	...
	//			GUILayout.Box ("", GUILayout.Height (2), GUILayout.Width (410));
		...
		//		GUILayout.Box ("", GUILayout.Height (2), GUILayout.Width (410));
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Shader based interact", GUILayout.MaxWidth (150f));
				script.ShaderBasedInteract = EditorGUILayout.Toggle (script.ShaderBasedInteract);
				EditorGUILayout.EndHorizontal ();
