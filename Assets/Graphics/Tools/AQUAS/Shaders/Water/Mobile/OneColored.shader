...
/////// Vectors:
                ...
                float3 _blend1 = saturate((_DeepWaterColor.rgb+(saturate((1 + (sceneZ-partZ) * (0.5 - 1) ) / (10.0)))));
...
/////// Diffuse:
				...
				float3 _lerp2 = lerp(lerp( _blend1, lerp(_ReflectionTex_var.rgb,_blend1,(1.0 - _Reflectionintensity)), _UseReflections),0,1);
                float3 fogColor = lerp( _lerp2, lerp(_lerp2,_FogColor.rgb,saturate(pow((distance(i.posWorld.rgb,_WorldSpaceCameraPos)/_FogDistance),_FogFade))), _EnableCustomFog );
...
////// Emissive:
                ...
                fogColor = _EnableCustomFog ? fogColor : 0;
/// Final Color:
                float3 finalColor = diffuse + specular + emissive + fogColor;
