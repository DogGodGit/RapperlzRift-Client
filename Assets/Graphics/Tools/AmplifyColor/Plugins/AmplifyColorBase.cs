...
public class AmplifyColorBase : MonoBehaviour
{
	...
	{ 
		get { return Mathf.Clamp01( effectVolumesBlendAdjust < 0.99f ? ( volumesBlendAmount - effectVolumesBlendAdjust ) / ( 1.0f - effectVolumesBlendAdjust ) : 1.0f ); } 
	}
	...
	void Update()
	{
		...
		if (TriggerVolumeProxy == null)
		{
			if (CsGameData.Instance.MyHeroTransform != null)
			{
				TriggerVolumeProxy = CsGameData.Instance.MyHeroTransform;
			}
		}
	...
	private int ComputeShaderPass()
	{
		...
		bool isHDR = false;//ownerCamera.hdr;
