...
public class PlanarReflection : MonoBehaviour
{
	...
	void OnWillRenderObject ()
	{
		...
		if (m_Iterations != 0)
		{
			RenderTexture rt = RenderTexture.GetTemporary (m_TextureSize, m_TextureSize, 0, m_ReflectionTexture.format);
			for (int i = 0; i < m_Iterations; i++)
			{
				float radius = (float)i * m_Interpolation + m_Interpolation;
				m_BlurMat.SetFloat ("_Radius", radius);
				Graphics.Blit (m_ReflectionTexture, rt, m_BlurMat, 0);
				Graphics.Blit (rt, m_ReflectionTexture, m_BlurMat, 1);
			}
	...
	void CreateMirrorObjects ()
	{
		...
		{
			GameObject go = new GameObject ("ER_Camera_" + GetInstanceID (), typeof (Camera), typeof (Skybox));
		...
		if (null == m_ReflectionTexture || m_OldReflectionTextureSize != m_TextureSize)
		{
			if (m_ReflectionTexture)
				DestroyImmediate (m_ReflectionTexture);
			m_ReflectionTexture = new RenderTexture (m_TextureSize, m_TextureSize, 16);
			m_ReflectionTexture.name = "ER_RT_" + GetInstanceID();
